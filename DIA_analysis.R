...
data_norm_df_1 <- data_norm_df[, c(1:5)]
proteins_MNAR_1 <- data_norm_df_1[rowSums(is.na(data_norm_df_1)) >= 3,] %>%
  pull(name) %>% 
  unique()

data_norm_df_2 <- data_norm_df[, c(1,6:9)]
proteins_MNAR_2 <- data_norm_df_2[rowSums(is.na(data_norm_df_2)) >= 3,] %>%
  pull(name) %>% 
  unique()


data_norm_df_3 <- data_norm_df[, c(1, 10:13)]
proteins_MNAR_3 <- data_norm_df_3[rowSums(is.na(data_norm_df_3)) >= 3,] %>%
  pull(name) %>% 
  unique()

data_norm_df_4 <- data_norm_df[, c(1, 14:17)]
proteins_MNAR_4 <- data_norm_df_4[rowSums(is.na(data_norm_df_4)) >= 3,] %>%
  pull(name) %>% 
  unique()

data_norm_df_5 <- data_norm_df[, c(1, 18:21)]
proteins_MNAR_5 <- data_norm_df_5[rowSums(is.na(data_norm_df_5)) >= 3,] %>%
  pull(name) %>% 
  unique()

data_norm_df_6 <- data_norm_df[, c(1, 22:25)]
proteins_MNAR_6 <- data_norm_df_6[rowSums(is.na(data_norm_df_6)) >= 3,] %>%
  pull(name) %>% 
  unique()

data_norm_df_7 <- data_norm_df[, c(1, 26:29)]
proteins_MNAR_7 <- data_norm_df_7[rowSums(is.na(data_norm_df_7)) >= 3,] %>%
  pull(name) %>% 
  unique()

data_norm_df_8 <- data_norm_df[, c(1, 30:33)]
proteins_MNAR_8 <- data_norm_df_8[rowSums(is.na(data_norm_df_8)) >= 3,] %>%
  pull(name) %>% 
  unique()

data_norm_df_9 <- data_norm_df[, c(1, 34:37)]
proteins_MNAR_9 <- data_norm_df_9[rowSums(is.na(data_norm_df_9)) >= 3,] %>%
  pull(name) %>% 
  unique()

data_norm_df_10 <- data_norm_df[, c(1, 38:41)]
proteins_MNAR_10 <- data_norm_df_10[rowSums(is.na(data_norm_df_10)) >= 3,] %>%
  pull(name) %>% 
  unique()

data_norm_df_11 <- data_norm_df[, c(1, 42:45)]
proteins_MNAR_11 <- data_norm_df_11[rowSums(is.na(data_norm_df_11)) >= 3,] %>%
  pull(name) %>% 
  unique()

data_norm_df_12 <- data_norm_df[, c(1, 46:49)]
proteins_MNAR_12 <- data_norm_df_12[rowSums(is.na(data_norm_df_12)) >= 3,] %>%
  pull(name) %>% 
  unique()

data_norm_df_13 <- data_norm_df[, c(1, 50:53)]
proteins_MNAR_13 <- data_norm_df_13[rowSums(is.na(data_norm_df_13)) >= 3,] %>%
  pull(name) %>% 
  unique()

data_norm_df_14 <- data_norm_df[, c(1, 54:57)]
proteins_MNAR_14 <- data_norm_df_14[rowSums(is.na(data_norm_df_14)) >= 3,] %>%
  pull(name) %>% 
  unique()

data_norm_df_15 <- data_norm_df[, c(1, 58:61)]
proteins_MNAR_15 <- data_norm_df_15[rowSums(is.na(data_norm_df_15)) >= 3,] %>%
  pull(name) %>% 
  unique()


MNAR_1 <- names(data_norm) %in% proteins_MNAR_1
MNAR_2 <- names(data_norm) %in% proteins_MNAR_2
MNAR_3 <- names(data_norm) %in% proteins_MNAR_3
MNAR_4 <- names(data_norm) %in% proteins_MNAR_4
MNAR_5 <- names(data_norm) %in% proteins_MNAR_5
MNAR_6 <- names(data_norm) %in% proteins_MNAR_6
MNAR_7 <- names(data_norm) %in% proteins_MNAR_7
MNAR_8 <- names(data_norm) %in% proteins_MNAR_8
MNAR_9 <- names(data_norm) %in% proteins_MNAR_9
MNAR_10 <- names(data_norm) %in% proteins_MNAR_10
MNAR_11 <- names(data_norm) %in% proteins_MNAR_11
MNAR_12 <- names(data_norm) %in% proteins_MNAR_12
MNAR_13 <- names(data_norm) %in% proteins_MNAR_13
MNAR_14 <- names(data_norm) %in% proteins_MNAR_14
MNAR_15 <- names(data_norm) %in% proteins_MNAR_15

data_norm_1 <- data_norm[, 1:4]
data_norm_2 <- data_norm[, 5:8]
data_norm_3 <- data_norm[, 9:12]
data_norm_4 <- data_norm[, 13:16]
data_norm_5 <- data_norm[, 17:20]
data_norm_6 <- data_norm[, 21:24]
data_norm_7 <- data_norm[, 25:28]
data_norm_8 <- data_norm[, 29:32]
data_norm_9 <- data_norm[, 33:36]
data_norm_10 <- data_norm[, 37:40]
data_norm_11 <- data_norm[, 41:44]
data_norm_12 <- data_norm[, 45:48]
data_norm_13 <- data_norm[, 49:52]
data_norm_14 <- data_norm[, 53:56]
data_norm_15 <- data_norm[, 57:60]


data_imp_1 <- DEP::impute(
  data_norm_1, 
  fun = "mixed",
  randna = !MNAR_1, # we have to define MAR which is the opposite of MNAR
  mar = "none", # imputation function for MAR
  mnar = "zero") # imputation function for MNAR

data_imp_2 <- DEP::impute(
  data_norm_2, 
  fun = "mixed",
  randna = !MNAR_2, 
  mar = "none", 
...
assay(data_imp) <- cbind(assay(data_imp_1), assay(data_imp_2), assay(data_imp_3),assay(data_imp_4),assay(data_imp_5),assay(data_imp_6),assay(data_imp_7),
                         assay(data_imp_8), assay(data_imp_9), assay(data_imp_10),  assay(data_imp_11), assay(data_imp_12), assay(data_imp_13), assay(data_imp_14), assay(data_imp_15))
...
data_diff_all <- test_diff(data_imp, type = "manual", test=c("wt_d4._vs_wt_d0.", "wt_d4._vs_wt_d1.", "wt_d4._vs_wt_d2.", "wt_d4._vs_wt_d3.", 
                                                             ...
                                                             "TET3KO_d0._vs_wt_d0.", "TET3KO_d1._vs_wt_d1.", "TET3KO_d2._vs_wt_d2.", "TET3KO_d3._vs_wt_d3.", "TET3KO_d4._vs_wt_d4.",
...
write.xlsx(xlsx, "C:\\Users\\Elisa\\Documents\\Post-doc\\Tet3 Project\\Tet3-MeCP2 project\\iNGNs project\\Proteomics_DIA_Franzi23122022\\Proteomics_iNGNs_DIA_R_DataOutput_impute_1+Gaussian_with-MeCP2.xlsx", sheetName = "Sheet1", colNames = TRUE, rowNames = TRUE)
write.xlsx(xlsx1, "C:\\Users\\Elisa\\Documents\\Post-doc\\Tet3 Project\\Tet3-MeCP2 project\\iNGNs project\\Proteomics_DIA_Franzi23122022\\Proteomics_iNGNs_DIA_R_DataOutput_Pre-imputation_with-MeCP2.xlsx", sheetName = "Sheet1", colNames = TRUE, rowNames = TRUE)
...
write.xlsx(xlsx2, "C:\\Users\\Elisa\\Documents\\Post-doc\\Tet3 Project\\Tet3-MeCP2 project\\iNGNs project\\Proteomics_DIA_Franzi23122022\\Proteomics_iNGNs_DIA_R_DataOutput_analysis_with-MeCP2.xlsx", sheetName = "Sheet1", colNames = TRUE, rowNames = TRUE)
...
plot_heatmap_Eli(dep, type = "centered", kmeans = TRUE, 
                 k = 8, col_limit = 2, show_row_names = FALSE, 
                 clustering_distance = "euclidean", clustering_method_rows = "ward.D2", row_dend_side = "left", row_dend_gp = gpar(lwd = 1), row_title_side = "left",
                 row_font_size = 15, col_font_size = 10, cluster_columns=FALSE) 
